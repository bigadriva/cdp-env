...
def create_catarinense_data():
    ...
    dados = pd.read_csv("/app/data/DADOS_DRIVA.TXT", sep=";", dtype={
                        'CNPJ': str, 'VAL_FAT': float}, decimal=",")
    vend = pd.read_csv("/app/data/VEND_DRIVA.TXT", sep=";")
    prod = pd.read_csv("/app/data/PROD_DRIVA.TXT", sep=";")
    ...
    dados_selected = dados_merge[['CNPJ', 'AREA_CLI', 'NOME_VEND_CLI',
                                'RAZAO SOCIAL', 'DATA', 'QTD_FAT', 'QTD_LOG_FAT', 'VAL_FAT', 'VAL_LOG_FAT', 'PRODUTO']]
